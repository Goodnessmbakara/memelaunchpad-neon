...
// Function to ensure sufficient balance
async function ensureSufficientBalance(address, targetBalance) {
    const maxAttempts = 10; // Maximum number of airdrop attempts
    let attempts = 0;
    ...
    while (attempts < maxAttempts) {
        ...
        console.log(`Requesting NEON airdrop (attempt ${attempts + 1}/${maxAttempts})...`);
        const success = await config.utils.airdropNEON(address);
        
        if (success) {
            // Wait for balance to update
            await new Promise(resolve => setTimeout(resolve, 5000));
            const newBalance = await ethers.provider.getBalance(address);
            console.log('New balance after airdrop:', ethers.formatEther(newBalance), 'NEON');
            
            if (newBalance >= targetBalance) {
                console.log('Target balance achieved after airdrop');
                return;
            }
        }
    ...
    if (finalBalance < targetBalance) {
        throw new Error(`Failed to achieve target balance after ${maxAttempts} attempts. Current balance: ${ethers.formatEther(finalBalance)} NEON`);
    }
}
...
describe('Test init', async function () {
    ...
    before(async function() {
        ...
        // We need at least 4000 NEON for all operations
        await ensureSufficientBalance(owner.address, ethers.parseEther("4000"));
